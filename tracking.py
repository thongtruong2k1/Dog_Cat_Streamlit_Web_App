...
# def convert_to_yolo(cls,x0,y0,x1,y1,image):
#     Width = image.shape[1]
...
def predict(image_file_name):
    ...
    check_labels = [2,4,5,7,8,14,15,16,20,21,22,24,33,41,46,49,50,60,63]
    ...
    image = img.copy()
    image, ratio, dwdh = letterbox(image, (640,640), auto=False)
    ...
    file_name,_=get_file_name(image_file_name)
    write_to_txt(boxes,'Result/labels/'+file_name+'.txt')

    # cv2.putText(ori_images[0],str(int(1/(time.time() - st))),(5, 20),cv2.FONT_HERSHEY_SIMPLEX,0.75,[225, 0, 0],thickness=4)
    # cv2.imshow('cc',cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
    cv2.imwrite('Result/images/'+file_name+'.jpg',cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
    # cv2.waitKey(0)
